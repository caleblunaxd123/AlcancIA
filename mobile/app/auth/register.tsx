...
export default function Register() {
  ...
  const createVerified = async (ticket: string) => {
    const result = await register({ name, email, password, ticket });
    if (!result.ok) { setChallenge(null); return setError(result.error); }
    // A brand-new cloud user: this device starts clean for them.
  ...
  return (
    <AuthShell compact eyebrow="Cuenta gratuita" title="Crea tu cuenta" subtitle="Empieza con tu correo. Sin costo y sin anuncios.">
      <View style={{ gap: theme.spacing.lg }}>
        <SocialAuthButtons onGoogleToken={social.onGoogleToken} onError={social.onError} dividerFirst={false} dividerLabel="o regístrate con tu correo" />
