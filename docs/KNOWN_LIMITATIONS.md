...
1c. **Verificación por correo sin cuentas en servidor.** Registro y recuperación exigen un código de 6 dígitos enviado por el backend (`/api/email/request|verify`, HMAC, 10 min, un solo uso, 5 intentos, 1 min entre envíos). Prueba que el correo es del usuario, pero la cuenta sigue viviendo solo en el dispositivo: el backend no emite sesión y la app confía en su propia respuesta "verified". Los desafíos viven en memoria (reiniciar la API invalida los códigos pendientes; una sola instancia). Con la base de datos de cuentas, la verificación debe emitir un token firmado que el servidor exija. Envío: Gmail SMTP con contraseña de aplicación en user-secrets (ver WORK_PROGRESS).
